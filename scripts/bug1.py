    ...
    while not rospy.is_shutdown():
        ...
        if state_ == 0:
            if regions_['front'] > 0.15 and regions_['front'] < 0.5:
                ...
                print(circumnavigate_starting_point_)
                change_state(1)
            ...
            if calc_dist_points(position_, desired_position_) < calc_dist_points(circumnavigate_closest_point_, desired_position_):
                circumnavigate_closest_point_ = position_
            #    print(calc_dist_points(circumnavigate_closest_point_, desired_position_))
                print("hey")
            # compare only after 5 seconds - need some time to get out of starting_point
            # if robot reaches (is close to) starting point
            if count_state_time_ > 5 and calc_dist_points(position_, circumnavigate_starting_point_) < 1:
                print("Hi")
                change_state(2)
            ...
            if calc_dist_points(position_, circumnavigate_closest_point_) < 0.2:
                change_state(0)
        if calc_dist_points(position_, desired_position_)<0.2:
            print("Yessss! we did it!")
        count_loop_ = count_loop_ + 1
        if count_loop_ == 20:
            count_state_time_ = count_state_time_ + 1
            count_loop_ = 0
        print("%f , %f" %(calc_dist_points(circumnavigate_closest_point_, desired_position_),calc_dist_points(position_, desired_position_)) )
        rate.sleep()
